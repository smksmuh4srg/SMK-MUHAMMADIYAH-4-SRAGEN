...
export default function SchoolLogo({
  ...
  const acronym = schoolInfo?.acronym || "SMK MUHESA";
  const customLogoUrl = schoolInfo?.customLogoUrl || "";
    ...
    <div className={`flex items-center gap-3 ${className}`} id="school-logo-container">
      ...
      <div className={`relative flex-shrink-0 ${dimensions[size]}`} id="school-logo-emblem">
        {customLogoUrl ? (
          <img
            src={customLogoUrl}
            alt={acronym}
            className="w-full h-full object-cover rounded-full border-2 border-oranye-primary shadow-xs"
          ...
          >
            ...
            <text fill="#ffffff" fontSize="5.5" fontWeight="bold" textAnchor="middle">
              <textPath href="#curve-text-path" startOffset="50%">
                {acronym.length > 15 ? acronym.substring(0, 15) : acronym}
              </textPath>
            </text>
      ...
      {showText && (
        <div className="flex flex-col select-none" id="school-logo-text">
          <span className={`font-black tracking-wider leading-tight uppercase ${textColorClass} ${
